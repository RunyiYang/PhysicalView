...
def main():
    ...
    ap.add_argument('sources',nargs='+',choices=['simany','trellis','menagerie','openpi'])
    ...
    for name in args.sources:
        spec=sources[name];dest=repo/spec['destination']
        if dest.exists():
            ...
            if head!=spec['revision'] or dirty:
                raise SystemExit(f'{dest}: existing checkout differs or has edits; preserved. Use a separate checkout or configure SIMANY_ROOT.')
            print(f'{name}: existing pinned checkout verified');continue
